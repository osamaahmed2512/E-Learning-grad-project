...
const AdminNavbar = () => {
  ...
  const menuContentRef = useRef(null);
    ...
    const isOutsideContent = menuContentRef.current && !menuContentRef.current.contains(event.target);
    
    if (isOutsideMenu && isOutsideButton && isOutsideContent) {
        setIsMenuOpen(false);
      }
    };
  ...
  const DropdownMenu = ({ items, onClose, isMobile }) => (
    <div 
      className="relative" 
      ref={menuContentRef}
  ...
  return (
    <header className={`
      ...
      border-b ${isDashboardPage ? 'bg-white' : 'bg-cyan-100'} 
      shadow-sm
    `}>
      <img
        src={assets.logo}
        ...
        {isAuthenticated ? (
          ...
          <div className="flex items-center gap-6">
            ...
            >
              ...
              <span className="relative invisible">Admin Login</span>
            </Link>
          </div>
        )}
      </div>
